...
# The number of entries should be the same number as the VAR with each array item counted individually
# e.g.: array[3] + var1 + var2 = 5 items
for (i in 1:datalink$entries) {
  ...
  d1a <- d4(datalink_offset) # always 1
  d1b <- d4(datalink_offset+4) # always 1
  d10 <- d4(datalink_offset+8) # always 0xa = 10
  d5 <- d2(datalink_offset+0xc) # always 5
  d32773 <- d2(datalink_offset+0xe) # always 0x8005 = 32773
  acq <- d4(datalink_offset+0x10) # Acquisition cycle (in milliseconds)
  d258 <- d2(datalink_offset+0x14) # always 0x102 = 256
  d0a <- d2(datalink_offset+0x16) # always 0
  d9 <- d4(datalink_offset+0x18) # always 9
  d0b <- d2(datalink_offset+0x1c) # Always 0
  # see get_dldt()
  data_type <- d1(datalink_offset+0x1e) # data_type

  # these two items represent the address of the tag
  database <- d2(datalink_offset+0x1f)
  database_offset <- d2(datalink_offset+0x21)

  data23 <- d1(datalink_offset+0x23)  
  data24 <- d2(datalink_offset+0x24)  
  data26 <- d2(datalink_offset+0x26)
  data28 <- d2(datalink_offset+0x28)
  array_size <- d2(datalink_offset+0x2a) # array size (usually 1 for a single primitive)
...
cat(
  ...
  # Page header
  paste('<h1>MiniWeb Server Language (MWSL) page, with ',tags$entries,' variables</h1>',sep=''),
  ...
  '</pre>',
  paste('<p>It means your FWX does not support HTTP variables, and you need to add the following to the Table Of Contents entries from which ',is_http,' exist:',sep=''),
  '<ol>',
  paste('<li><code>WEBLINK exists</code> in the file? <b>',is_weblink,'</b>',sep=''),
  paste('<li><code>FUNC_STARTPROC</code> exists in the file? <b>',is_startproc,'</b>',sep=''),
  '</ol>',
...
for(i in 1:(tags$entries)) {
  ...
  for (j in 1:(arr_size)) {
    ...
    if (datalink$d0a == 0) {
      db <- datalink$DB
      dboff <- datalink$DBOFFSET
      dbchar <- get_dldt(datalink$data_type)
      # extract address
      address <- paste('DB',db,'DB',dbchar,dboff,sep='')
      if (dbchar == 'X') {
        address <- paste(address,'.0',sep='')
      }
    } else {
      address <- ''
    }
    # extractio acquisition time
    acq <- datalink$acq
    ...
    writeLines(tr, con=html.handle, sep='', useBytes = TRUE)
  }
}
